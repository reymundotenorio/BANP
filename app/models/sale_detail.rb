class SaleDetail < ApplicationRecord
  ...
  def self.search_returns(search)
    if search
      self.joins(:sale).joins(:product).where("(products.name LIKE :search OR products.name_spanish LIKE :search)", search: "%#{search}%").returned.not_zero
  ...
  scope :not_zero, -> { where("(sale_details.quantity > 0)") }
  ## End Scopes
end
